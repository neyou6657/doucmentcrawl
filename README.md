...
这是一个功能强大的Tampermonkey（油猴）脚本，用于自动爬取网页开发者文档，支持左侧链接扫描、自动分类和智能内容提取，**根据URL结构保存为ZIP文件夹**。

## 功能特性

### 📦 ZIP文件夹导出 **[v1.2.0 NEW]**
- **基于URL的文件夹结构**：根据网页URL自动创建文件夹层次
- **独立文件保存**：每个页面保存为单独的HTML和TXT文件
- **一键打包下载**：所有文件打包成一个ZIP文件
- **自动索引**：生成INDEX.md文件列出所有页面
- **完整元数据**：保存源URL和抓取时间
...
### 🎯 内容提取（简化版）
- **innerHTML提取**：完整保存页面HTML内容
- **textContent提取**：纯文本内容
- **智能识别**：自动查找主内容区域
- **高效快速**：简化提取逻辑，提高性能

### 💾 导出格式
- **ZIP格式**（推荐）：文件夹结构，包含HTML和TXT文件
- **JSON格式**：完整的结构化数据（用于调试）
...
**文件说明：**
- **HTML文件**：包含完整的页面内容（innerHTML），带有元数据（源URL、抓取时间）
- **TXT文件**：纯文本内容（textContent），便于搜索和阅读
- **INDEX.md**：Markdown格式的索引，列出所有页面及其位置
- **文件夹结构**：完全遵循原网站的URL路径结构

**示例：**
- `https://example.com/guide/getting-started` → `guide/getting-started.html`
- `https://example.com/api/reference/methods` → `api/reference/methods.html`
